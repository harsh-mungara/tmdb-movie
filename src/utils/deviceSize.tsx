...
export function getDeviceSize(width, height, platform) {
  if (platform === 'ios') {
    if ((width <= 320 && height <= 480) || (width <= 480 && height <= 320)) {
      // iphone 4 spec
      return DeviceSizeConst.XSMALL;
    } else if (
      (width <= 320 && height <= 568) ||
      (width <= 568 && height <= 320)
    ) {
      // iphone 5 spec
      return DeviceSizeConst.SMALL;
    } else if (
      (width <= 375 && height <= 667) ||
      (width <= 667 && height <= 375)
    ) {
      // iphone 6 spec
      return DeviceSizeConst.NORMAL;
    } else if (
      (width <= 414 && height <= 736) ||
      (width <= 736 && height <= 414)
    ) {
      // iphone 6 plus spec
      return DeviceSizeConst.LARGE;
    }
    // greater then inphone 6 plus, such as tablets
    return DeviceSizeConst.XLARGE;
    ...
    if (height - ExtraDimensions.getSoftMenuBarHeight() < 640) {
      return DeviceSizeConst.SMALL;
    } else {
      return DeviceSizeConst.NORMAL;
    }
